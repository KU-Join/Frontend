...
const SignUpForm = () => {
  ...
  const handleSubmit = (e: any) => {
    ...
    if (
      person.signup_pwd.search(/[0-9]/) < 0 ||
      person.signup_pwd.search(/[a-z]/) < 0
    ) {
      e.preventDefault();
    ...
    if (
      person.signup_pwd != person.verify_pwd &&
      person.signup_pwd.length > 0 &&
      person.verify_pwd.length > 0
    ) {
      e.preventDefault();
    ...
    fetch(API_URL + '/member-service/members', {
      ...
      .then((response) => {
        if (
          response.email === person.signup_id &&
          response.nickname === person.signup_name
        ) {
          router.push('./finishsignup');
        } else {
          alert(
            '회원가입에 실패했습니다. 아이디와 닉네임을 다시 확인해주세요.'
          );
        }
      });
  ...
  const SendMail = () => {
    fetch(API_URL + '/member-service/mail/send', {
      ...
      .then((response) => response.json())
      .then((response) => {
        response.message === '메일을 전송하였습니다.'
          ? setModalIsOpen(true)
          : alert(
              '메일 전송실패. 아이디를 확인해주세요. 이미 가입된 사용자일 수 있습니다.'
            );
      });
  };
  ...
  return (
    <Container>
      ...
      <Form>
        <WrapForm>
          <LabelID htmlFor="signup_id" id="signup_id_label">
            포탈 아이디
          </LabelID>
          <InputID
            type="text"
            ...
            name="signup_id"
            placeholder=""
            value={person.signup_id}
            onChange={handleInputChange}
          ></InputID>
          <DuplicateBtn
            type="button"
            id="duplicate_check_btn"
            onClick={SendMail}
          >
            중복 확인
          </DuplicateBtn>
          <LabelPwd htmlFor="signup_pwd" id="signup_pwd_label">
            비밀번호
          </LabelPwd>
          <InputPwd
            type="password"
            ...
            name="signup_pwd"
            placeholder=""
            value={person.signup_pwd}
            onChange={handleInputChange}
          ></InputPwd>
          <LabelVerifyPwd htmlFor="verify_pwd" id="verify_pwd_label">
            비밀번호 확인
          </LabelVerifyPwd>
          <InputVerifyPwd
            type="password"
            ...
            name="verify_pwd"
            placeholder=""
            value={person.verify_pwd}
            onChange={handleInputChange}
          ></InputVerifyPwd>
          <LabelName htmlFor="signup_name" id="signup_name_label">
            이름(닉네임)
          </LabelName>
          <InputName
            type="text"
